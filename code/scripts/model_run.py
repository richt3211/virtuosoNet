...
class ModelRun():
    ...
    def __init__(self, args):
        ...
        self.args = args
        LOSS_TYPE = self.args.trainingLoss
        HIERARCHY = False
        IN_HIER = False
        HIER_MEAS = False
        HIER_BEAT = False
        HIER_MODEL = None
        RAND_TRAIN = self.args.randomTrain
        TRILL = False
        ...
        if self.args.sessMode == 'train':
            ...
            (self.learning_rate, self.TIME_STEPS, self.DELTA_WEIGHT, WEIGHT_DECAY, self.GRAD_CLIP, self.KLD_MAX, self.KLD_SIG))
        ...
        if HIERARCHY:
            ...
            NUM_INPUT += self.NUM_PRIME_PARAM
            NUM_OUTPUT = 5
        else:
            NUM_OUTPUT = 11
        if IN_HIER:
        ...
        is_trill_index_concated = -11 - (self.NUM_PRIME_PARAM + num_second_param)
        ...
        with open(self.args.dataName + "_stat.dat", "rb") as f:
            ...
            if self.args.trainingLoss == 'CE':
                ...
                for i in range(self.NUM_PRIME_PARAM, self.NUM_PRIME_PARAM + num_trill_param - 1):
                    new_trill_param += len(BINS[i]) - 1
                self.NUM_PRIME_PARAM = new_prime_param
                print('New NUM_PRIME_PARAM: ', self.NUM_PRIME_PARAM)
                num_trill_param = new_trill_param + 1
                NUM_OUTPUT = self.NUM_PRIME_PARAM + num_trill_param
    ...
    def save_checkpoint(self,state, is_best, filename=self.args.modelCode, model_name='prime'):
    ...
    def load_file_and_generate_performance(self,path_name, composer=args.composer, z=args.latent, 
    ...
    def encode_performance_style_vector(self,input, input_y, edges, note_locations, model=MODEL):
        with torch.no_grad():
    ...
    def run_model_in_steps(self,input, input_y, edges, note_locations, initial_z=False, model=MODEL):
    ...
    def batch_time_step_run(self,data, model, batch_size=batch_size):
